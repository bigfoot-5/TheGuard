def score_format_compliance(case: dict, generated_text: str) -> float:
    ...
    """
    # Extract the limit from the dataset row (default to 160 for WhatsApp if missing)
...
if __name__ == "__main__":
    ...
    print(f"Perfect (<160): {score_format_compliance(mock_case, perfect_copy)}")  # Expected: 1.0
    ...
    print(f"Slightly Over (174): {score_format_compliance(mock_case, slight_over)}")  # Expected: 0.8
    ...
    print(f"Moderately Over (198): {score_format_compliance(mock_case, moderate_over)}")  # Expected: 0.5
    
    # Test 4: Complete failure (250+ chars)
    fail_copy = perfect_copy + " " + perfect_copy
    print(f"Total Failure (>200): {score_format_compliance(mock_case, fail_copy)}")  # Expected: 0.0