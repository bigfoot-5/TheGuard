...
from prompt_manager import save_prompt_version, update_prompt_status
...
def save_eval_results(averages_dict: dict, raw_arrays_dict: dict, provider: str, status: str, failed_cases: dict = None, cost_data: dict = None, latency: float = 0.0, task_providers: dict = None, task_prompt_hashes: dict = None):
...
def main():
    ...
    current_raw_arrays, current_averages = {}, {}
    metric_types, metric_case_ids = {}, {}
    total_pipeline_cost = 0.0
    task_costs_dict, raw_csv_data, task_providers_dict = {}, [], {}
    ...
    for task_name, task_config in config["tasks"].items():
        ...
        task_ids = [c.get("deal_id") or c.get("cart_id") or c.get("merchant_id") or f"#{i+1}" for i, c in enumerate(cases)]
        ...
        prompt_hash = save_prompt_version(
            prompt_name=f"auto_{task_name}", 
            prompt_content=system_prompt, 
            author="CI_CD_Runner", 
            model_config={"model": model, "temperature": temp}
        )
        active_prompt_hashes.append(prompt_hash)
        task_prompt_hashes[task_name] = prompt_hash
        ...
        for case in cases:
            ...
            csv_row = {"Task": task_name, "Case_ID": case_id, "Model_Used": model, "Prompt_Hash": prompt_hash}
            ...
            for metric in task_config["metrics"]:
                reg_key = metric["registry_key"]
                result = REGISTRY[reg_key](case, output, **metric)
                score, eval_cost = (result[0], result[1]) if isinstance(result, tuple) else (result, 0.0)
            ...
            task_cost += case_cost
            if "gemini" in model.lower() or "llama" in model.lower(): time.sleep(2)
    ...
    actual_providers = " & ".join(set([task.get("model", global_model) for task in config["tasks"].values()]))
    ...
    if not baseline_raw_arrays:
        ...
        save_eval_results(current_averages, current_raw_arrays, actual_providers, "GO (BASELINE)", cost_data=task_costs_dict, latency=total_execution_time, task_providers=task_providers_dict, task_prompt_hashes=task_prompt_hashes)
        for row in raw_csv_data: row["Regression_Detected"] = "False"
        csv_path = os.path.join(PROJECT_ROOT, "eval_report_raw_custom.csv")
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["Task", "Case_ID", "Model_Used", "Regression_Detected", "Total_Cost_USD", "Latency_Seconds"] + list(raw_csv_data[0].keys())[4:])
            writer.writeheader(); writer.writerows(raw_csv_data)
        for h in active_prompt_hashes: update_prompt_status(h, "GO (BASELINE)")
        sys.exit(0)
        
    print("\n📊 Executing Phase 5: Statistical Comparison Engine...")
    final_decision, failure_reasons, failed_cases_dict = "GO", [], {}

    for reg_key, data_type_info in metric_types.items():
        if reg_key not in baseline_raw_arrays: continue
        stats_result = calculate_paired_bootstrap(baseline_raw_arrays[reg_key], current_raw_arrays[reg_key]) if data_type_info["type"] == "continuous" else calculate_mcnemar(baseline_raw_arrays[reg_key], current_raw_arrays[reg_key])
        metric_decision = evaluate_decision_gate(stats_result, metric_type=data_type_info["type"])
        if metric_decision == "NO-GO":
            final_decision = "NO-GO"
            regressions = [metric_case_ids[reg_key][idx] for idx, (b, c) in enumerate(zip(baseline_raw_arrays[reg_key], current_raw_arrays[reg_key])) if c < b]
        ...
        elif metric_decision == "INCONCLUSIVE" and final_decision != "NO-GO": final_decision = "GO (STABLE)"

    for row in raw_csv_data:
        row["Regression_Detected"] = "False"
        for m_name in row.keys():
    ...
    if final_decision == "NO-GO":
        ...
        save_eval_results(current_averages, current_raw_arrays, actual_providers, "NO-GO", failed_cases_dict, task_costs_dict, total_execution_time, task_providers_dict, task_prompt_hashes)
        sys.exit(1)
    elif final_decision == "GO (STABLE)":
        print(f"✅ GO (STABLE): No statistically significant difference detected. Safe to merge.")
        save_eval_results(current_averages, current_raw_arrays, actual_providers, "GO (STABLE)", cost_data=task_costs_dict, latency=total_execution_time, task_providers=task_providers_dict, task_prompt_hashes=task_prompt_hashes)
        sys.exit(0)
    else:
        print(f"✅ GO (IMPROVED): Pipeline detected statistically significant improvements!")
        save_eval_results(current_averages, current_raw_arrays, actual_providers, "GO (IMPROVED)", cost_data=task_costs_dict, latency=total_execution_time, task_providers=task_providers_dict, task_prompt_hashes=task_prompt_hashes)
        sys.exit(0)
